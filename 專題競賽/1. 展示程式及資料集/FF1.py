...
# 計數原始和新分數的0、1、2、3數量
count_true = {0: 0, 1: 0, 2: 0, 3: 0}
count_pred = {0: 0, 1: 0, 2: 0, 3: 0}
...
def process_score(score, invalid_count_name, out_of_range_count_name, count_dict):
    ...
    if score in [None, ""]:  # 處理空字串或 None 的情況
        ...
        if score_int in count_dict:  # 計數 0, 1, 2, 3 分
            count_dict[score_int] += 1
        return score_int
    return 0
...
print(f"  1 分: {count_true[1]}")
print(f"  2 分: {count_true[2]}")
print(f"  3 分: {count_true[3]}")

print("模型預測數據中的分數數量：")
print(f"  0 分: {count_pred[0]}")
print(f"  1 分: {count_pred[1]}")
print(f"  2 分: {count_pred[2]}")
print(f"  3 分: {count_pred[3]}")
