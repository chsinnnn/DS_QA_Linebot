...
# 計數原始和新分數的0、1-2、2-3數量
count_true = {0: 0, "1-2": 0, "2-3": 0}
count_pred = {0: 0, "1-2": 0, "2-3": 0}
...
# 處理分數的函數，將 1 和 2 合併為 "1-2" 且 2 和 3 合併為 "2-3"
def process_score(score, invalid_count_name, out_of_range_count_name, count_dict):
    ...
    if score in [None, ""]:  # 處理空字串或 None 的情況
        ...
        # 將 3 的分數合併為 "2-3"
            ...
            score_label = 0
        if score_label in count_dict:  # 計數 0, "1-2", "2-3" 分
...
# 輸出原始數據中的分數數量
print("原始數據中的分數數量：")
print(f"  0 分: {count_true[0]}")
print(f"  1-2 分: {count_true['1-2']}")
print(f"  2-3 分: {count_true['2-3']}")

print("\n模型預測數據中的分數數量：")
print(f"  0 分: {count_pred[0]}")
print(f"  1-2 分: {count_pred['1-2']}")
print(f"  2-3 分: {count_pred['2-3']}")

# 輸出無效和範圍外的計數
print(f"\n無效的真實分數: {invalid_true_count}, 無效的預測分數: {invalid_pred_count}")
print(f"範圍外的真實分數: {out_of_range_true_count}, 範圍外的預測分數: {out_of_range_pred_count}")

# 計算混淆矩陣（合併後的分數）
conf_matrix = confusion_matrix(y_true, y_pred, labels=[0, "1-2", "2-3"])

# 使用 pandas 顯示混淆矩陣表格
conf_matrix_df = pd.DataFrame(conf_matrix, index=[f"True {i}" for i in [0, "1-2", "2-3"]],
                              columns=[f"Pred {i}" for i in [0, "1-2", "2-3"]])

print("\n混淆矩陣：")
print(conf_matrix_df)

# 輸出每個分數的結果，包括 Score 0
for label in labels:
